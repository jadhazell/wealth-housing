...
def extract_unmerged_lease_data(unmerged_lease_data_file, zip_data_directory):
	...
	for index, row in tqdm(leases.iterrows()):
		address = row["description"].replace(".","").replace(",","").replace("'","").replace("'","")
...
# 	if os.path.isdir(postcode_directory) and "lease_properties.p" in os.listdir(postcode_directory):
...
for postcode in os.listdir(zip_data_directory_2):
