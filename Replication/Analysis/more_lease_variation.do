...
// 	graph export "$FIGURES/100_year_bucket_results_fe`count'_$tag.png", replace
	...
	// GMS split
	cap drop bucket 
	gen     bucket = 1 if leasehold & lease_duration_at_trans < 80
	replace bucket = 2 if leasehold & lease_duration_at_trans >= 80  & lease_duration_at_trans < 100
	replace bucket = 3 if leasehold & lease_duration_at_trans >= 100 & lease_duration_at_trans < 125
	replace bucket = 4 if leasehold & lease_duration_at_trans >= 125 & lease_duration_at_trans < 150
	replace bucket = 5 if leasehold & lease_duration_at_trans >= 150 & lease_duration_at_trans <= 300
	replace bucket = 6 if leasehold & lease_duration_at_trans >= 700
...
// 	esttab using "$TABLES/summary_gmsbucket_$tag.tex", main(mean) aux(sd) nonumber nostar unstack label noobs nonote title("Stats By Year of Purchase \label{tab: summary gms fe`count'_$tag}") varlabels(1 "80-100" 2 "100-125" 3 "125-150" 4 "150-300" 5 "700-1000") replace substitute(\_ _)
	...
	di "RUNNING: eststo: reghdfe $dep_var i.`bucket_name'##c.$indep_var if obs_to_use`count', absorb(`fe') cluster($cluster)"
...
// 		gaps replace substitute(\_ _)
	...
	twoway (rarea ub lb xaxis) (line coeff xaxis), xlabel(1 "0-80" 2 "80-100" 3 "100-125" 4 "125-150" 5 "150-300" 6 "700-1000" 7 "Freehold", angle(45)) yline(0)
	graph export "$FIGURES/results_gmsbucket_fe`count'_$tag.png", replace

// 	// Tighter split (i.e. smaller buckets)
// 	cap drop bucket 
// 	gen bucket     = 1 if leasehold & lease_duration_at_trans >= 80  & lease_duration_at_trans < 100
